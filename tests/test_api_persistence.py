...
class McpTurnPlanTests(ApiPersistenceTestCase):
    ...
    def test_mcp_state_exposes_greenery_summary(self):
        ...
        for field in [
            ...
            self.assertNotIn(field, state)
        self.assertNotIn("arrival_plan", state)
    ...
    def test_mcp_actions_expose_turn6_food_preorder_packages(self):
        ...
        actions = game_api.mcp_available_actions()["available_actions"]
        actions = [item for item in actions if item["action"] != "restart_game"]
...
class PublicStateHidingTests(ApiPersistenceTestCase):
    def test_public_states_hide_arrival_plan_and_future_traffic_predictions(self):
        self.engine.state.today_arrival_plan = [{
            "arrival_status": "pending",
            "arrival_turn": 2,
            "group_size": 4,
        ...
        for state in (api_state, mcp_state):
            ...
            self.assertNotIn("arrival_plan", state)
            self.assertNotIn("pending_by_turn", dumped)
            self.assertNotIn("pending_groups", dumped)

    def test_frontend_morning_reservations_use_confirmed_reservation_facts(self):
        source = open(
            "camping_plaza/frontend/scripts/overview.js", encoding="utf-8"
        ).read()
