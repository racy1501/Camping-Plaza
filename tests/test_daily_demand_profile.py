...
class DailyDemandHelperTests(unittest.TestCase):
    ...
    def test_daily_demand_profile_has_only_the_multi_reservation_generation_flag(self):
        engine = make_engine()
        engine.state.daily_demand_profile = None
        engine.state.daily_demand_profile_day = 0

        with mock.patch("game_engine.random.random", side_effect=AssertionError("profile must not roll reservations")):
            with mock.patch.object(engine, "_calculate_day_guest_demand", return_value=7):
                with mock.patch.object(engine, "_calculate_overnight_guest_demand", return_value=4):
                    profile = engine._ensure_daily_demand_profile()

        self.assertFalse(profile["reservations_processed"])
        self.assertNotIn("reservation_request_available", profile)
        self.assertNotIn("reservation_visit_type", profile)
        self.assertNotIn("reservation_group_size", profile)

    def test_reservation_generation_uses_ten_independent_day_opportunities(self):
        engine = make_engine()
        ...
        with mock.patch.object(engine, "_assign_hidden_tags"), \
             mock.patch("game_engine.random.random", side_effect=[0.1] + [0.99] * 9 + [0.99]), \
             mock.patch("game_engine.random.randint", return_value=6):
            engine._generate_daily_reservation()

        self.assertEqual(len(engine.state.reservations), 1)
        self.assertEqual(engine.state.reservations[0]["visit_type"], "day")
        self.assertEqual(engine.state.reservations[0]["group_size"], 6)

    def test_reservation_generation_uses_each_unlocked_tent_opportunity(self):
        engine = make_engine()
        engine.state.daily_demand_profile = None
        engine.state.daily_demand_profile_day = engine.state.day
        ...
        with mock.patch.object(engine, "_assign_hidden_tags"), \
             mock.patch("game_engine.random.random", side_effect=[0.99] * 10 + [0.1, 0.1]), \
             mock.patch("game_engine.random.randint", side_effect=[2, 2]):
            engine._generate_daily_reservation()

        overnight = [r for r in engine.state.reservations if r["visit_type"] == "overnight"]
        self.assertEqual(len(overnight), 2)
        self.assertEqual([r["tent_id"] for r in overnight], [1, 2])

    def test_reservation_group_size_uses_one_to_six(self):
        engine = make_engine()
        for tent in engine.tents.values():
            tent.is_unlocked = False
        engine.tents[1].is_unlocked = True
        ...
        with mock.patch.object(engine, "_assign_hidden_tags"), \
             mock.patch("game_engine.random.random", side_effect=[0.1] + [0.99] * 20), \
             mock.patch("game_engine.random.randint", return_value=6) as randint_mock:
            engine._generate_daily_reservation()
    ...
    def test_same_day_daily_demand_profile_does_not_reroll_reservation(self):
        ...
        with mock.patch("game_engine.random.random", side_effect=AssertionError("profile must not roll reservations")) as random_mock:
            with mock.patch("game_engine.random.randint", side_effect=AssertionError("profile must not choose group size")) as randint_mock:
                with mock.patch.object(engine, "_calculate_day_guest_demand", return_value=7):
                    with mock.patch.object(engine, "_calculate_overnight_guest_demand", return_value=4):
                        first_profile = engine._ensure_daily_demand_profile()
                        second_profile = engine._ensure_daily_demand_profile()
    ...
    def test_new_day_daily_demand_profile_rerolls_reservation(self):
        ...
        self.assertNotEqual(first_profile, second_profile)
        self.assertFalse(second_profile["reservations_processed"])
        self.assertNotIn("reservation_request_available", second_profile)
        random_mock.assert_not_called()
        randint_mock.assert_not_called()

    def test_natural_demand_profile_does_not_depend_on_reservation_rolls(self):
        ...
        self.assertEqual(profile_a["natural_day_group_demand"], 7)
        self.assertEqual(profile_b["natural_day_group_demand"], 8)
        self.assertEqual(profile_a["natural_overnight_group_demand"], 4)
        self.assertEqual(profile_b["natural_overnight_group_demand"], 5)
        self.assertFalse(profile_a["reservations_processed"])
        self.assertFalse(profile_b["reservations_processed"])
    ...
    def test_day_reservation_auto_creates_accepted_record_and_charges_campsite_fee(self):
        ...
        with mock.patch.object(engine, "_assign_hidden_tags", side_effect=assign_hidden_tags), \
             mock.patch("game_engine.random.random", side_effect=[0.1] + [0.99] * 10), \
             mock.patch("game_engine.random.randint", return_value=4):
            engine._generate_daily_reservation()

        self.assertEqual(len(engine.state.reservations), 1)
        reservation = engine.state.reservations[0]
        self.assertEqual(reservation["visit_type"], "day")
        self.assertEqual(reservation["group_size"], 4)
        ...
        self.assertIn("npc_id", reservation)
        self.assertEqual(reservation["economic_level"], 2)
        self.assertEqual(reservation["spending_habit"], 1)
    ...
    def test_day_reservation_same_day_repeat_processing_does_not_duplicate_charge(self):
        ...
        with mock.patch.object(engine, "_assign_hidden_tags"), \
             mock.patch("game_engine.random.random", side_effect=[0.1] + [0.99] * 10), \
             mock.patch("game_engine.random.randint", return_value=3):
            engine._generate_daily_reservation()
            first_reservation = [dict(r) for r in engine.state.reservations]
            engine._generate_daily_reservation()

        self.assertEqual(engine.state.balance, 1000 + engine.CAMPSITE_FEE)
        self.assertEqual(engine.state.today_income["campsite"], engine.CAMPSITE_FEE)
        self.assertEqual(len(engine.state.today_events), 1)
        self.assertEqual(engine.state.reservations, first_reservation)
    ...
    def test_process_planned_arrivals_does_not_create_duplicate_overnight_guest(self):
        engine = make_engine()
        engine.state.day = 2
        engine.state.turn = 2
        engine.state.reservations = [{
    ...
    def test_overnight_reservation_auto_selects_smallest_suitable_tent(self):
        ...
        with mock.patch.object(engine, "_assign_hidden_tags"), \
             mock.patch("game_engine.random.random", side_effect=[0.99] * 10 + [0.1, 0.99, 0.99]), \
             mock.patch("game_engine.random.randint", return_value=2):
            engine._generate_daily_reservation()

        self.assertEqual(len(engine.state.reservations), 1)
        self.assertEqual(engine.state.reservations[0]["visit_type"], "overnight")
    ...
    def test_overnight_reservation_selects_smallest_id_when_capacities_tie(self):
        ...
        with mock.patch.object(engine, "_assign_hidden_tags"), \
             mock.patch("game_engine.random.random", side_effect=[0.99] * 10 + [0.1, 0.99]), \
             mock.patch("game_engine.random.randint", return_value=4):
            engine._generate_daily_reservation()

        self.assertEqual(engine.state.reservations[0]["tent_id"], 4)
    ...
    def test_overnight_reservation_does_not_overwrite_current_tent_status_or_occupied_by(self):
        ...
        self.assertEqual(engine.state.reservations[0]["tent_id"], 2)
    ...
    def test_overnight_reservation_same_day_repeat_processing_does_not_duplicate_charge_or_lock(self):
        ...
        with mock.patch.object(engine, "_assign_hidden_tags"), \
             ...
             mock.patch("game_engine.random.randint", return_value=2):
            engine._generate_daily_reservation()
            first_reservations = [dict(r) for r in engine.state.reservations]
            engine._generate_daily_reservation()

        self.assertEqual(engine.state.balance, 1000 + engine.TENT_PRICES[1])
        self.assertEqual(engine.state.today_income["accommodation"], engine.TENT_PRICES[1])
        self.assertEqual(len(engine.state.today_events), 1)
        self.assertEqual(engine.state.reservations, first_reservations)
    ...
    def test_day_reservation_logic_still_charges_campsite_fee_normally(self):
        ...
        self.assertEqual(engine.state.balance, 1000 + engine.CAMPSITE_FEE)
        self.assertEqual(engine.state.today_income["campsite"], engine.CAMPSITE_FEE)
        self.assertEqual(engine.state.reservations[0].get("tent_id"), None)
    ...
    def test_management_quality_uses_default_then_true_average_rating_ratio(self):
        engine = make_engine()
